...
namespace CnD.CommunalPayments.Server.Api.Endpoints.InvoiceEndpoints
{
    public class InvoceEndpoint : EndpointDefinition
    {
        ...
        {
            app.MapGet("/api/invoices/get-invoices", GetAllInvoces);
            app.MapGet("/api/invoices/get-invoice/{id:int}", GetInvoceById);
        ...
        private async Task<IResult> GetAllInvoces([FromServices] IInvoiceService service, [FromServices] IMapper mapper, HttpContext context)
        {
        ...
        // [Authorize(AuthenticationSchemes = AuthData.AuthSchemes)]
        [FeatureGroupName("Invoices")]
        private async Task<IResult> GetInvoceById([FromServices] IInvoiceService service, [FromServices] IMapper mapper, int id)
        {
            if (id <= 0)
